...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		// add stock	
		// top nav child bar 
		$('#topNavAddStock').addClass('active');	
		...
		$("#createStockForm").unbind('submit').bind('submit', function() {
			...
			// form validation 
			if(stockDate == "") {
				$("#stockDate").after('<p class="text-danger"> The Stock Date field is required </p>');
			...
			if(empName == "") {
				$("#empName").after('<p class="text-danger"> The Employee Name field is required </p>');
			...
			if(empContact == "") {
				$("#empContact").after('<p class="text-danger"> The Contact field is required </p>');
				$('#empContact').closest('.form-group').addClass('has-error');
			} else {
				$('#empContact').closest('.form-group').addClass('has-success');
			} // /else
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(stockDate && empName && empContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	' <br /> <br /> <a type="button" onclick="printStock('+response.stock_id+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> Print </a>'+
	            	'<a href="stocks.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Stock </a>'+
		...
		// top nav child bar 
		$('#topNavManageStock').addClass('active');
...
function printStock(stockId = null) {
	if(stockId) {		
			
		$.ajax({
			url: 'php_action/printStock.php',
			type: 'post',
        ...
        mywindow.document.write('<html><head><title>Stock Invoice</title>');        
...
setTimeout(function() {
    ...
    mywindow.close();
		...
		}); // /ajax function to fetch the printable stock
	} // /if stockId
} // /print stock function
		...
		$("#removeStockBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
	          ...
	          $(".alert-success").delay(500).show(10, function() {
						...
						// error messages
						$(".removeStockMessages").html('<div class="alert alert-warning">'+
	            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
	            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> '+ response.messages +
	          '</div>');
			...
			});  // /ajax function to remove the stock
...
// Payment ORDER
function paymentStock(stockId = null) {
	if(stockId) {

		$("#stockDate").datepicker();

		$.ajax({
			url: 'php_action/fetchStockData.php',
			type: 'post',
			data: {stockId: stockId},
			dataType: 'json',
			success:function(response) {				
				...
				// due 
				$("#due").val(response.stock[10]);				

				// pay amount 
				$("#payAmount").val(response.stock[10]);

				var paidAmount = response.stock[9] 
				var dueAmount = response.stock[10];							
				var grandTotal = response.stock[8];

				// update payment
				$("#updatePaymentStockBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
						$("#updatePaymentStockBtn").button('loading');
						$.ajax({
			          ...
			          // refresh the manage stock table
								manageStockTable.ajax.reload(null, false);
