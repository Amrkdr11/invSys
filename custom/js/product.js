...
$(document).ready(function() {
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			if(stateName == "") {
				$("#stateName").after('<p class="text-danger">State Name field is required</p>');
				$('#stateName').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#stateName").find('.text-danger').remove();
				// success out for form 
				$("#stateName").closest('.form-group').addClass('has-success');	  	
			}	// /else
			...
			if(productImage && productName && quantity && brandName && categoryName && productStatus && stateName) {
