...
} else if($_GET['s'] == 'manord') { 
	echo "<div class='div-request div-hide'>manord</div>";
} else if($_GET['s'] == 'editOrd') { 
...
<ol class="breadcrumb">
  ...
  <li class="active">
  	<?php if($_GET['s'] == 'add') { ?>
  		Add Stock
		<?php } else if($_GET['s'] == 'manord') { ?>
			Manage Stock
		<?php } // /else manage stock ?>
  </li>
</ol>
...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<?php } else if($_GET['s'] == 'manord') { ?>
			<i class="glyphicon glyphicon-edit"></i> Manage Stock
		<?php } else if($_GET['s'] == 'editOrd') { ?>
			<i class="glyphicon glyphicon-edit"></i> Edit Stock
		<?php } ?>
	...
	<div class="panel-body">
		...
		<?php if($_GET['s'] == 'add') { 
  		...
  		<form class="form-horizontal" method="POST" action="php_action/createStock.php" id="createStockForm">
			  ...
			  <div class="form-group">
			    <label for="stockDate" class="col-sm-2 control-label">Stock Date</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="stockDate" name="stockDate" autocomplete="off" />
			    </div>
			  </div> <!--/form-group-->
			  <div class="form-group">
			    <label for="empName" class="col-sm-2 control-label">Employee Name</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="empName" name="empName" placeholder="Employee Name" autocomplete="off" />
			    </div>
			  </div> <!--/form-group-->
			  <div class="form-group">
			    <label for="empContact" class="col-sm-2 control-label">Employee Contact</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="empContact" name="empContact" placeholder="Contact Number" autocomplete="off" />
			    </div>
			  </div> <!--/form-group-->			  
			  ...
			  <div class="col-md-6">
				  ...
				  <div class="form-group">
				    <label for="empContact" class="col-sm-3 control-label">Payment Type</label>
				    <div class="col-sm-9">
				  ...
				  <div class="form-group">
				    <label for="empContact" class="col-sm-3 control-label">Payment Status</label>
				    <div class="col-sm-9">
				  ...
				  <div class="form-group">
				    <label for="empContact" class="col-sm-3 control-label">Payment Place</label>
				    <div class="col-sm-9">
			  ...
			  <div class="form-group submitButtonFooter">
			    ...
			    <button type="button" class="btn btn-success" onclick="addRow()" id="addRowBtn" data-loading-text="Loading..."> <i class="glyphicon glyphicon-plus-sign"></i> Add Row </button>
			      ...
			      <button type="reset" class="btn btn-danger" onclick="resetStockForm()"><i class="glyphicon glyphicon-erase"></i> Reset</button>
			    </div>
			  </div>
			</form>
		<?php } else if($_GET['s'] == 'manord') { 
			...
			<table class="table table-hover table-striped table-bordered" id="manageStockTable">
				<thead>
					<tr>
						<th>#</th>
						<th>Stock Date</th>
		...
		// /else manage stock
  		...
  		<form class="form-horizontal" method="POST" action="php_action/editStock.php" id="editStockForm">
  			...
  			$sql = "SELECT stocks.stocks_id, stocks.stocks_date, stocks.emp_name, stocks.emp_contact, stocks.sub_total, stokcs.vat, stocks.total_amount, stocks.discount, stocks.grand_total, stocks.paid, stokcs.due, stocks.payment_type, stocks.payment_status,stocks.payment_place,stocks.gstn FROM stocks 	
					WHERE stocks.stokcs_id = {$stocksId}";
			  ...
			  <div class="form-group">
			    <label for="empName" class="col-sm-2 control-label">Employee Name</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="empName" name="empName" placeholder="Employee Name" autocomplete="off" value="<?php echo $data[2] ?>" />
			    </div>
			  </div> <!--/form-group-->
			  <div class="form-group">
			    <label for="empContact" class="col-sm-2 control-label">Employee Contact</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="empContact" name="empcontact" placeholder="Contact Number" autocomplete="off" value="<?php echo $data[3] ?>" />
			    </div>
			  </div> <!--/form-group-->			  
			  ...
			  <table class="table" id="productTable">
			  	...
			  	<tbody>
			  		...
			  		$stockItemSql = "SELECT stock_item.stock_item_id, stock_item.stock_id, stock_item.product_id, stock_item.quantity, stock_item.rate, stock_item.total FROM stock_item WHERE stock_item.stock_id = {$stockId}";
						$stockItemResult = $connect->query($stockItemSql);
						// $stockItemData = $stockItemResult->fetch_all();						
						
						// print_r($stockItemData);
			  		$arrayNumber = 0;
			  		// for($x = 1; $x <= count($stockItemData); $x++) {
			  		$x = 1;
			  		while($stockItemData = $stockItemResult->fetch_array()) { 
			  			// print_r($stockItemData); ?>
			  			<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">			  				
			  				<td style="margin-left:20px;">
			  					<div class="form-group">
			  					...
			  					<select class="form-control" name="productName[]" id="productName<?php echo $x; ?>" onchange="getProductData(<?php echo $x; ?>)" >
			  						...
			  						<?php
			  							...
			  							while($row = $productData->fetch_array()) {									 		
			  								$selected = "";
			  								if($row['product_id'] == $stockItemData['product_id']) {
			  									$selected = "selected";
			  								} else {
			  									$selected = "";
			  								}
			  				...
			  				<td style="padding-left:20px;">			  					
			  					<input type="text" name="rate[]" id="rate<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" value="<?php echo $stockItemData['rate']; ?>" />			  					
			  					<input type="hidden" name="rateValue[]" id="rateValue<?php echo $x; ?>" autocomplete="off" class="form-control" value="<?php echo $stockItemData['rate']; ?>" />			  					
			  				</td>
							<td style="padding-left:20px;">
			  					<div class="form-group">
									<?php
			  				...
			  				<td style="padding-left:20px;">
			  					<div class="form-group">
			  					<input type="number" name="quantity[]" id="quantity<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control" min="1" value="<?php echo $stockItemData['quantity']; ?>" />
			  					</div>
			  				</td>
			  				<td style="padding-left:20px;">			  					
			  					<input type="text" name="total[]" id="total<?php echo $x; ?>" autocomplete="off" class="form-control" disabled="true" value="<?php echo $stockItemData['total']; ?>"/>			  					
			  					<input type="hidden" name="totalValue[]" id="totalValue<?php echo $x; ?>" autocomplete="off" class="form-control" value="<?php echo $stockItemData['total']; ?>"/>			  					
			  				</td>
			  				<td>
			  ...
			  <div class="form-group editButtonFooter">
			    ...
			    <input type="hidden" name="stockId" id="stockId" value="<?php echo $_GET['i']; ?>" />

			    <button type="submit" id="editStockBtn" data-loading-text="Loading..." class="btn btn-success"><i class="glyphicon glyphicon-ok-sign"></i> Save Changes</button>
...
<!-- edit stock -->
<div class="modal fade" tabindex="-1" role="dialog" id="paymentStockModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body form-horizontal" style="max-height:500px; overflow:auto;" >
      	...
      	<div class="paymentStockMessages"></div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="updatePaymentStockBtn" data-loading-text="Loading..."> <i class="glyphicon glyphicon-ok-sign"></i> Save changes</button>	
      </div>           
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
